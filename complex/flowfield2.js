...
class Agent {
  ...
  draw() {
    let r = map(sin(frameCount / 2), -1, 1, 255, 255); 
    let g = map(sin(frameCount / 2), -1, 1, 100, 200); 
    let b = map(cos(frameCount / 2), -1, 1, 50, 100);  

    push();
    stroke(r, b, g, 80);
    strokeWeight(1);
    line(this.lastPosition.x, this.lastPosition.y * 1, this.position.x, this.position.y * 0);
    pop();
  }
}
